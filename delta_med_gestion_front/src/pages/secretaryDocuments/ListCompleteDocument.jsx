...
const ListCompleteDocument = () => {
    const [ documents, setDocuments ] = useState(null)
    const [ loaded, setLoaded ] = useState(false)
    const [ err, setErr ] = useState(null)

    useEffect(() => {
        secretaryFileService.getAllSecretaryFiles()
            .then(res => {
                ...
                <table className="min-w-full text-sm text-left rtl:text-right text-gray-700">
                    ...
                    <tr key={index} className="bg-white dark:bg-gray-800">
                        ...
                        <div className="flex flex-wrap gap-2">
                            <a href={`/secretarydocuments/one/${document._id}`} className="inline-flex items-center justify-center text-white bg-green-700 hover:bg-green-800 focus:outline-none focus:ring-4 focus:ring-green-300 font-medium rounded-lg text-xs sm:text-sm px-3 py-2 dark:bg-green-600 dark:hover:bg-green-700 dark:focus:ring-green-800">Voir</a>
                        </div>
                        </td>
                    </tr>
                    ))}
...
export default ListCompleteDocument;