...
const UpdateMutuelle = () => {
    ...
    const onSubmit = (e) => {
        e.preventDefault();
        healthInsuranceService.updateHealthInsurance(id, {
            name: infos.name.trim(),
        })
        .then(res => {
            navigate('/mutuelle/list');
        })
        .catch(error => {
            console.error(error);
    ...
    <div>
        ...
        <div className='text-center mt-5'>
            <a className='focus:outline-none text-white bg-yellow-400 hover:bg-yellow-500 focus:ring-4 focus:ring-yellow-300 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 dark:focus:ring-yellow-900' href="/mutuelle/list">Retour</a>
        </div>
    </div>
    );
};

export default UpdateMutuelle;