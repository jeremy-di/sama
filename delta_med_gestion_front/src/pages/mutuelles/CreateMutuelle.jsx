...
const CreateMutuelle = () => {
    ...
    const onSubmit = (e) => {
        e.preventDefault()
        healthInsuranceService.newHealthInsurance(infos)
            .then(res => {
                navigate('/mutuelle')
            })
            .catch(error => {
    ...
    return (
        <div>
            <h1 className='text-4xl text-center m-5 text-sky-800'>Ajout d'une mutuelle</h1>
            <div className="border sm:w-full lg:w-1/2 m-auto mt-5 p-5 rounded border-sky-800">
                {/* Form */}
                <form className="max-w-md mx-auto" onSubmit={onSubmit}>
                
                <div className="mb-5">
                    <label htmlFor="name" className="block mb-2 text-sm font-medium text-gray-900 dark:text-gray-400">Nom de la mutuelle</label>
