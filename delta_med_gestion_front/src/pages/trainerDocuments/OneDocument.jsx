...
const OneDocument = () => {
    ...
    useEffect(() => {
        trainerFileService.getOneTrainerFile(id)
            .then(res => {
    ...
    const handledelete = async () => {
        try {
            setDeleting(true)
            await trainerFileService.deleteTrainerFile(document._id)
            navigate("/trainerdocuments")
        } catch (error) {
            setDeleting(false)
        ...
        return (
            <div>
                ...
                <div className='border border-sky-800 w-3/4 m-auto p-2'>
                    <iframe
                        title={document.designation || 'Document PDF'}
                        src={`${document.name}#toolbar=1&navpanes=0&scrollbar=1`}
